...
final class APIManager: APIManaging {
    ...
    /// The method execute() can handle any Decodable type (Value). That type is passed inside a Request<Value> wrapper (like Request<Page<Movie>>, Request<GenreList>, etc).
    ...
    func execute<Value: Decodable>(_ request: Request<Value>, completion: @escaping (Result<Value, APIError>) -> Void) {
        urlSession.dataTask(with: urlRequest(for: request)) { responseData, response, _ in
            if let data = responseData {
                ...
                /// When the network call completes, it calls: completion(.success(decodedValue)) where decodedValue (i.e. response) returned is of type Value.
